...
TYPES: BEGIN OF ty_list,
         ogrencino TYPE zogrencino,
         kitapno   TYPE zfk_de_kitapno,
       END OF ty_list.
...
PARAMETERS : p_list  AS LISTBOX VISIBLE LENGTH 15,
             p_list2 AS LISTBOX VISIBLE LENGTH 15.
...
ENDFORM.
*&---------------------------------------------------------------------*
*& Form F_DROPDOWN_LIST
*&---------------------------------------------------------------------*
FORM f_dropdown_list .
  SELECT kitapno FROM zfk_kitap INTO CORRESPONDING FIELDS OF TABLE it_list UP TO 10 ROWS.

  LOOP AT it_list INTO s_list.
    value-key = s_list-kitapno.
    value-text = s_list-kitapno.
  ...
  CLEAR: it_list,list.
  SELECT ogrencino FROM zfk_ogrenci INTO CORRESPONDING FIELDS OF TABLE it_list UP TO 10 ROWS.

  LOOP AT it_list INTO s_list.
    value-key = s_list-ogrencino.
    value-text = s_list-ogrencino.
    APPEND value TO list.
  ENDLOOP.
